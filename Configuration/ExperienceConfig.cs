...
public static class ExperienceConfig
{
    ...
    public static void Initialize()
    {
        ...
        ExperienceSystem.XpGainedMessageTemplate = _configFile.Bind("Templates", "XP Gained Message Template", "<color=#ffff00>You gain {xpGained} XP by slaying a Lv.{mobLevel} enemy.</color> [ XP: <color=white>{earned}</color>/<color=white>{needed}</color> ]", "Sets the template to format the message displayed when the player gains XP points. Allowed placeholders: {xpGained}, {mobLevel}, {earned}, {needed}.").Value;
        ExperienceSystem.XpLostMessageTemplate = _configFile.Bind("Templates", "XP Lost Message Template", "You've been defeated, <color=white>{xpLost}</color> XP is lost. [ XP: <color=white>{earned}</color>/<color=white>{needed}</color> ]", "Sets the template to format the message displayed when the player loses XP points. Allowed placeholders: {xpLost}, {earned}, {needed}.").Value;
        ExperienceSystem.LevelUpMessageTemplate = _configFile.Bind("Templates", "Level Up Message Template", "<color=#ffff00>Level up! You're now level</color> <color=white>{level}</color><color=#ffff00>!</color>", "Sets the template to format the message displayed when the player levels up. Allowed placeholders: {level}.").Value;
    }
}